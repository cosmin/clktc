...
class ViewTests(TestCase):
    ...
    def test_adding_a_link_creates_link(self):
        self.client.post('/add/', {"destination_url" : EXAMPLE_URL, "short_url" : "example"})
    ...
    def test_adding_a_link_redirects_to_all_links_page(self):
        response = self.client.post('/add/', {"destination_url" : EXAMPLE_URL, "short_url" : "example"})
    ...
    def test_save_on_edit_link_updates_link_with_new_details(self):
        link = given_a_link()
        self.client.post("/edit/%s" % link.pk, {"destination_url" : "http://example.org", "short_url" : "example2"})
